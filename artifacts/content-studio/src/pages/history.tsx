    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">{data?.total ?? 0} pieces generated</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-10 text-center text-muted-foreground">No content found</Card>
      ) : (
        <div className="space-y-3">
              ...
              {expandedId === item.id && (
                <div className="border-t px-4 py-4 bg-muted/30 space-y-3">
                  {item.titles?.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Viral Titles</p>
                      {item.titles.slice(0, 2).map((t: any, i: number) => (
                        <p key={i} className="text-sm text-foreground">{t.title} <span className="text-muted-foreground">({t.viralityScore}/100)</span></p>
                      ))}
                    </div>
                  )}
                  {item.script && (
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Script Preview</p>
                      <p className="text-sm text-muted-foreground line-clamp-4">{item.script}</p>
                    </div>
                  )}
