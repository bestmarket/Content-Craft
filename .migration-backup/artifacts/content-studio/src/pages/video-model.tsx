...
export default function VideoModel() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Video Modeler</h1>
        <p className="text-slate-500 text-sm mt-1">Paste up to 5 viral video links and model a superior script</p>
      </div>
      ...
      <Card className="p-6 border space-y-5">
        ...
        <div>
          ...
          {videoUrls.length < 5 && (
            <Button variant="ghost" size="sm" onClick={addUrl} className="mt-2 text-purple-600">
      ...
      {result && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <Card className="p-6 border">
            <h2 className="text-sm font-semibold text-slate-700 mb-3">Modeled Viral Titles</h2>
            <div className="space-y-2">
              {result.titles?.map((t: any, i: number) => (
                <div key={i} className="flex items-start justify-between gap-3 p-3 rounded-lg border hover:bg-slate-50 transition-colors">
          ...
          <Card className="p-6 border">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-slate-700">Modeled Script</h2>
              <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(result.script ?? ""); }}>
                <Copy className="w-4 h-4 mr-1" /> Copy
              </Button>
            </div>
            <Textarea value={result.script ?? ""} readOnly className="min-h-64 text-sm font-mono bg-slate-50" />
